...
import { useAppDispatch } from "../../../../../hook/auth";
import {
  fetchDictionary,
  clearDictionary,
...
type WordDetailProps = {
  dictionary?: DictionaryWord | null;
};
...
const WordDetail = ({ dictionary }: { dictionary: DictionaryWord | null }) => {
  if (!dictionary) {
    return (
      <Box
        sx={{
...
const DictionaryLibrary = () => {
  ...
  return (
    <Box sx={{ m: 2 }}>
      <BackArrow onClick={() => navigate("/library")} />
      <Box
        sx={{
          position: "fixed",
          top: "50%",
